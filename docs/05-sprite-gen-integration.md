...
- [ ] 딸기 옷 고양이 정체성 고정 (빨간 딸기 몸통+씨 점·초록 꼭지·빼꼼 데드팬 얼굴·작은 다리)
...
**베이스 생성 프롬프트 (딸기 노아 — 외부 이미지 생성기용)**:
```text
2.5-head chibi pixel-art game sprite mascot: a small cat with a flat deadpan face
peeking out of a red strawberry costume (round strawberry body with seed dots,
small green leaf/stem on top), tiny stubby cat legs poking out,
thick dark 1-2px outline, limited palette, flat cel shading (one highlight + one shadow),
single clean idle pose facing the camera, full body inside frame,
readable silhouette at small size, flat solid blue (#0000FF) background,
no drop shadow, no glow, no text — base game sprite, not an illustration
```
...
- **크로마키**: 딸기 노아는 **빨강(몸통) + 초록(꼭지)** 둘 다 주요색 → magenta(빨강 먹힘)·green(꼭지 먹힘) 둘 다 부적합. **blue(#0000FF) 키 권장**(피사체에 파랑 없음). 추출 후 빨강·초록이 둘 다 살아남았는지 확인. *(auto는 거리 기반이라 blue 계열을 고르는지 확인 권장)*
...
python3 "$SG/scripts/prepare_sprite_run.py" \
  ...
  --description "deadpan cat peeking from a red strawberry costume (green stem), tiny legs; 2.5-head pixel mascot" \
  --request "$REQ" \
  --chroma-key "#0000FF" --cell-size 256 --safe-margin 24 --force   # 딸기=빨강+초록 → blue 키 (추출 후 두 색 다 확인)
