...
- **크로마키**: 검은 고양이 → `--chroma-key auto`(베이스 샘플링) 권장. magenta/green을 강제하면 노아 액센트색과 hue 인접 시 색이 먹힐 수 있음. *(베이스 없이 auto는 magenta로 fallback — 실제 베이스를 줘야 샘플링됨)*
...
python3 "$SG/scripts/prepare_sprite_run.py" \
  ...
  --description "cute aloof/tsundere black cat; 2.5-head pixel mascot" \
  --request "$REQ" \
  --chroma-key auto --cell-size 256 --safe-margin 24 --force
